...
- `origin` = personal fork `https://github.com/paul-bouzian/t3code.git`
- `upstream` = source repository `https://github.com/pingdotgg/t3code.git`

Goal:
- bring the fork up to date with `upstream/main`
...
2. Summarize divergence between `upstream/main` and `origin/main` using `git log --left-right --cherry-pick --oneline upstream/main...origin/main`.
3. If the current working tree is dirty, prefer creating a separate worktree for the sync branch.
4. Create a short-lived branch from `origin/main` named `sync/<sync_label>-upstream-main`.
5. Merge `upstream/main` into that branch. Do not commit directly on `main`.
...
7. For each PR number in `upstream_pr_numbers`:
   ...
   - fetch the PR head with `git fetch upstream pull/<number>/head:pr/<number>`
   - inspect the commits that are not already in `upstream/main`
